...
void SerialReadTest(int com_id){
	printf("########Gateway for Multi-robot System(Receive)########\n");
...
uint16_t CRC16(uint8_t *pData, uint16_t nData) {
	uint8_t nCRCHi = 0xFF;
	uint8_t nCRCLo = 0xFF;
	uint8_t nIndex;
	while (nData--) {
...
void SerialWriteTest(int com_id){
	printf("########Gateway for Multi-robot System(Send)########\n");
	...
	unsigned char writedata[36] = { 0x7e, 0x45, 0x20, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0xa5, 0xa5, \
		0x00, 0x00, 0x00, 0x00, 0x30, 0x7b, 0xcf, 0xa5, 0xa5, 0xa5, 0xa5, 0xa5, 0xa5, 0xa5, 0xa5, 0xa5,\
		0xa5, 0xa5, 0xa5, 0xa5, 0x00, 0x00, 0x00, 0x00, 0x0b };
	...
	while (SP->IsConnected())
	{
		...
		int ret = SP->WriteData((char *)writedata, sizeof(writedata));
		if (ret){
